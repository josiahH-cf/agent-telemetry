...
CATALOG: tuple[dict[str, Any], ...] = (
    ...
    _metric('workspace_accounting_tokens', 'Workspace measured usage',
        'Tokens and API-equivalent dollars attributed to a reporting group (workspace) in a period, with a Shared/Unassigned portion that closes the partition.',
        'Start from private per-repository period rows. A managed native session whose exact run portions, repository and runs all resolve to one group stays whole there; otherwise each exact run portion follows its recorded Project, the session remainder and whole-session dollars stay with the owners only when they agree, and repository usage outside managed sessions follows the repository when exactly one declared repository resolves to it and that repository has exactly one group. Groups plus Shared/Unassigned equal the period total.',
        ['restricted observatory.sqlite3 portfolio_daily', 'sessions', 'usage_observations', 'private outcome_events', 'consumer reporting map'],
        'The reporting map is the first-party consumer\'s explicit grouping, not an inference. Dollars are never split by token share: tokens without dollars at a scope are counted separately. Repository and task rows overlap the group total. Unobserved activity is a coverage gap, never an amount.', 'tokens, API-equivalent USD', 'machine-only', 'derived'),
    _metric('run_usage_allocation', 'Run usage allocation',
        'The part of a native session\'s measured usage exactly bounded to one outcome run, and the state of the rest.',
        'Turn-scoped usage receipts are exact portions. Cumulative receipts yield exact interval deltas only from a runtime-started session baseline and while counters do not decrease; a reset or missing counter makes the session partial. Portions must not exceed the measured session. A session bound to one outcome with no usage receipts is exclusive and whole. Otherwise allocation is unknown.',
        ['private outcome_events', 'sessions'], 'No run cost is estimated from elapsed time, message counts or equal division. An outcome\'s usage is the union over all its attempts. Historical receipts without an explicit counter scope stay unknown.', 'tokens', 'machine-only', 'derived'),
    _metric('contribution_share', 'Contribution share',
        'A group\'s share of the period\'s measured tokens, or of the period\'s priced API-equivalent dollars.',
        'token_share = group tokens / all measured tokens in the period including Shared/Unassigned; dollar_share = group priced dollars / all priced dollars in the period. Null when the denominator is zero or the group has no priced dollars at this scope.',
        ['restricted observatory.sqlite3 portfolio_daily', 'private outcome_events'], 'The two shares have different denominators and neither is subscription-allowance consumption.', 'ratio', 'machine-only', 'derived'),
    _metric('review_acceptance_rate', 'Reviewed acceptance rate',
        'Accepted outcomes among outcomes with an explicit review verdict.',
        'COUNT latest verdict=accepted / COUNT latest verdict in (accepted, needs-changes); null when no outcome has a verdict. Delivered-but-unreviewed outcomes are reported beside it and never enter either side.',
        ['private outcome_events'], 'Not a quality score; unreviewed work is neither accepted nor rejected.', 'ratio', 'machine-only', 'derived'),
    _metric('cohort_tokens_per_delivered_outcome', 'Tokens per delivered outcome',
        'Exactly attributed run tokens divided by delivered outcomes, only for a cohort whose every outcome has exact usage attribution.',
        'SUM attributed tokens over the cohort / COUNT delivered; null with a stated reason when any outcome is partial, unknown or unbound, or when nothing was delivered.',
        ['private outcome_events', 'sessions'], 'Descriptive only; mixed work kinds, workflows, models and policies do not establish efficiency or value.', 'tokens per outcome', 'machine-only', 'derived'),
    _metric(
        "lifetime_sessions",
    ...
    _metric(
        "window_unpriced_tokens",
        "Window unpriced tokens",
        "Observed tokens in the selected inclusive UTC window excluded from exact dollars because no exact model-price row matched.",
        "SUM private per-source daily unpriced_tokens WHERE date >= window.from AND date <= window.to, priced per UTC day, source, vendor and project with the collector's exact-model rows.",
        ["restricted observatory.sqlite3 portfolio_daily", "prices.json"],
        "Private consumer measurement; never folded into exact dollars and never a complete zero-cost claim while unpriced or unobserved sources remain.",
        "tokens",
        "machine-only",
    ),
    _metric(
        "window_sessions",
        "Window sessions",
        "Distinct provider sessions with at least one usage event in the selected inclusive UTC window.",
        "COUNT DISTINCT vendor plus provider session id over private per-source daily session presences WHERE date >= window.from AND date <= window.to.",
        ["restricted observatory.sqlite3 portfolio_session_days"],
        "Private consumer measurement; distinct from window_session_days, which counts one session on two UTC dates twice, and from a session's lifetime values.",
        "sessions",
        "machine-only",
    ),
    _metric(
