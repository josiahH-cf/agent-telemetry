...
| ST-14 | hygiene | Commit author and committer metadata exposed a personal address. | **hardened** — repo-local identity uses the GitHub noreply address; global configuration is unchanged. Historical identity remediation is gated by bundle and tree-equivalence proof. | Fresh commits, full-history identity audit, commit-count comparison, and per-commit tree comparison are the completion checks. |
...
desired default for unrelated repositories. A pre-rewrite bundle and exact
tree-hash comparison protect the narrow historical identity rewrite.
