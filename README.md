...
Optional `subscriptions.local.json` is ignored by Git. It may contain `{"monthly_usd":{"provider":0}}`; when present, its total is divided by observed accepted features. When absent or invalid, the dashboard names the subscription view as unavailable instead of guessing.

## Durability

`data/history/daily-YYYY-MM-DD.json` and `data/history/cost-YYYY-MM-DD.json` are durable daily series. `data/rounds.json` merges by `(spec, numeric round)` and never deletes an old record.
...
| `metrics.worth.subscription_amortization` | Optional local subscription status, USD per accepted feature, and generated reason. |
| `metrics.cost.vendors.<vendor>` | Machine sessions, tokens, API-equivalent USD, unpriced tokens, observed models, and anonymous loop/other split. |
...
The suite covers v1 adapters, partial lines, incomplete seals, numeric ordering, immutable history, both vendor content sentinels, incremental snapshot/cumulative semantics, both price formulas, cached-input and reasoning subsets, exact-model refusal, non-loop anonymity, publish guard state, six-section local rendering, and a seeded scrub violation. Provider usage is never refreshed; stale or missing data stays visibly named.
